...
create_maintenance_schedule() {
    local schedule_file
    schedule_file="${SCHEDULES_DIR}/maintenance_schedule_$(date +%Y%m%d).md"
...
execute_incremental_cleanup() {
...

    {
        ...
        local old_logs
        old_logs=$(find "${CODE_DIR}/Tools" -name "*.log" -mtime +30 2>/dev/null | wc -l)
        ...
        local temp_files
        temp_files=$(find /tmp -name "workflow_*" -o -name "recent_*" 2>/dev/null | wc -l)
        ...
        local disk_usage
        disk_usage=$(du -sh "${CODE_DIR}/Tools" 2>/dev/null | cut -f1)
...
generate_disaster_recovery() {
    local recovery_file
    recovery_file="${SCHEDULES_DIR}/disaster_recovery_$(date +%Y%m%d).md"

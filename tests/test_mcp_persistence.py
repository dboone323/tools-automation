...
def test_execute_task_writes_json(tmp_path, monkeypatch):
    ...
    # create a dummy task
    task = {'id': 'task_persist_1', 'agent': 't', 'command': 'echo', 'project': None, 'status': 'running'}
    ...
    persisted = os.path.join(os.path.dirname(server.__file__), 'tasks', f"{task['id']}.json")
...
def test_run_server_loads_tasks(tmp_path, monkeypatch):
    ...
    with open(os.path.join(tasks_dir_path, 'task_existing.json'), 'w', encoding='utf-8') as f:
        json.dump(sample, f)

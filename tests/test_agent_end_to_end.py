...
spec = importlib.util.spec_from_file_location('mcp_server', str(automation_dir / 'mcp_server.py'))
...
"""
    ...
    # register an agent
    r = requests.post(f'{url}/register', json={'agent': 'e2e-agent', 'capabilities': []}, timeout=5)
    assert r.status_code == 200

    # enqueue and execute the task
    r = requests.post(f'{url}/run', json={'agent': 'e2e-agent', 'command': 'write-artifact', 'execute': True}, timeout=5)
    assert r.status_code == 200
    data = r.json()

#!/bin/bash
# Auto-generated test for .auto_restart_agent_performance_monitor.sh
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MARKER_FILE="$SCRIPT_DIR/agents/.auto_restart_agent_performance_monitor.sh"
AGENT_FILE="$SCRIPT_DIR/agents/agent_performance_monitor.sh"

# Source test framework
source "$SCRIPT_DIR/shell_test_framework.sh"

run_tests() {
    announce_test "check_marker__auto_restart_agent_performance_monitor"
    ...
    test_passed "check_marker__auto_restart_agent_performance_monitor"
}

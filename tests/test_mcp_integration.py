...
def test_server_executes_allowed_command(tmp_path, monkeypatch):
    ...
    # register agent
    r = requests.post(f'{url}/register', json={'agent': 'int-test', 'capabilities': []}, timeout=5)
    assert r.status_code == 200

    # request run
    r = requests.post(f'{url}/run', json={'agent': 'int-test', 'command': 'test-echo', 'execute': True}, timeout=5)
    assert r.status_code == 200
    data = r.json()

...
test.describe('Agent Dashboard E2E Tests', () => {
  ...
  test('loads main dashboard with all sections', async ({ page }) => {
    ...
    await expect(page.locator('text=⚙️ Agent Status')).toBeVisible();
    await expect(page.locator('text=🎯 Task Analytics')).toBeVisible();
    await expect(page.locator('text=🧠 AI/ML Performance')).toBeVisible();

    // Check analytics section
    await expect(page.locator('text=📈 Umami Analytics Overview')).toBeVisible();
  });
  ...
  test('displays server status correctly', async ({ page }) => {
    ...
    await expect(serverStatus).toContainText('✅ Running');
  });
  ...
  test('handles server offline gracefully', async ({ page }) => {
    ...
    const serverStatus = page.locator('#server-status');
    await expect(serverStatus).toContainText('❌ Not Running');

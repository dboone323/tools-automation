#!/bin/bash
# Auto-generated test for .auto_restart_agent_cleanup.sh
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MARKER_FILE="$SCRIPT_DIR/agents/.auto_restart_agent_cleanup.sh"
AGENT_FILE="$SCRIPT_DIR/agents/agent_cleanup.sh"

# Source test framework
source "$SCRIPT_DIR/shell_test_framework.sh"

run_tests() {
    announce_test "check_marker__auto_restart_agent_cleanup"
    ...
    test_passed "check_marker__auto_restart_agent_cleanup"
}

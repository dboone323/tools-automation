...
def test_agent_auto_restore(tmp_path):
    ...
    try:
        ...
        agent_proc = subprocess.Popen([venv_python, AGENT_PY, '--name', 'pytest-auto', '--capabilities', 'execute', '--interval', '1'], env=env)
        time.sleep(0.5)

        # register and enqueue modify-fail execute against the test MCP URL
        base = f'http://127.0.0.1:{port}'
        r = requests.post(f'{base}/register', json={'agent': 'pytest-auto', 'capabilities': ['execute']}, timeout=3)
        assert r.ok
        r2 = requests.post(f'{base}/run', json={'agent': 'pytest-auto', 'command': 'modify-fail', 'project': '', 'execute': True}, timeout=3)
        assert r2.ok

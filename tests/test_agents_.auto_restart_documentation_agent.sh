...
AGENT_NAME="test_agents_.auto_restart_documentation_agent.sh"
...
#!/bin/bash
# Auto-generated test for .auto_restart_documentation_agent.sh
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MARKER_FILE="$SCRIPT_DIR/agents/.auto_restart_documentation_agent.sh"
AGENT_FILE="$SCRIPT_DIR/agents/documentation_agent.sh"

# Source test framework
source "$SCRIPT_DIR/shell_test_framework.sh"

run_tests() {
    announce_test "check_marker__auto_restart_documentation_agent"
    ...
    test_passed "check_marker__auto_restart_documentation_agent"
}

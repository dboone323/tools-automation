...
push_project_workflows() {
    ...
    workflow_changes=$(git status --porcelain .github/workflows/ 2>/dev/null || true)
    ...
    if [[ -z ${workflow_changes} ]]; then
        ...
        staged_workflows=$(git diff --cached --name-only .github/workflows/ 2>/dev/null || true)
        if [[ -n ${staged_workflows} ]]; then
            ...
            local unpushed
            unpushed=$(git log origin/main..HEAD --oneline 2>/dev/null || git log --oneline -1 2>/dev/null || true)
            if [[ -n ${unpushed} ]]; then
        ...
        # shellcheck disable=SC2001  # sed is more appropriate for adding indentation to multi-line strings

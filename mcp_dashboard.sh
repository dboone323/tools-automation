...
case "$cmd" in
  ...
  show)
    tid=${2:-}
    if [[ -z "$tid" ]]; then
      echo "Usage: $0 show <task_id>" >&2; exit 2
    fi
  ...
  artifacts)
    ...
    case "$sub" in
      ...
      show)
        pkg=${3:-}
        if [[ -z "$pkg" ]]; then
          echo "Usage: $0 artifacts show <package.tar.gz>" >&2; exit 2
        fi
        if [[ -f "$ART_DIR/$pkg" ]]; then
          tar -tzf "$ART_DIR/$pkg" | sed -n '1,200p'
        else
          echo "Package not found: $pkg" >&2; exit 1
        fi
        ;;
      download)
        pkg=${3:-}
        outdir=${4:-$(pwd)}
        if [[ -z "$pkg" ]]; then
          echo "Usage: $0 artifacts download <package.tar.gz> [outdir]" >&2; exit 2
        fi
        if [[ -f "$ART_DIR/$pkg" ]]; then
        ...
        ;;
      *)
        echo "Usage: $0 artifacts [list|show <pkg>|download <pkg> [outdir]]" >&2; exit 2
        ;;
    esac
    ;;
  *)
    echo "Usage: $0 [list|show <task_id>|artifacts]" >&2; exit 2
    ;;
esac

    ...
    def do_GET(self):
        ...
        if self.path.startswith('/api/plugins/list'):
                ...
                with open(POLICY_CONF) as f:
                    ...
                    for line in f:
                        ...
                        if line == '[plugins]':
                            ...
                            allow_list = [x.strip() for x in line.split('=',1)[1].split(',')]
                        elif section == 'plugins' and line.startswith('block='):
                            block_list = [x.strip() for x in line.split('=',1)[1].split(',')]
            except Exception:
                pass
            if plugin in block_list:
                ...
                self.end_headers()
                with open(AUDIT_LOG, 'a') as f:
                    f.write(f"[{now}] user={user} action=api_run_plugin plugin={plugin} result=fail reason=policy_blocked\n")
                return
            if plugin not in allow_list:
                self.send_response(403)
                self.end_headers()
                with open(AUDIT_LOG, 'a') as f:
                    f.write(f"[{now}] user={user} action=api_run_plugin plugin={plugin} result=fail reason=policy_not_allowed\n")
                return
            if token != (API_TOKEN or 'changeme'):
                self.send_response(403)
                self.end_headers()
                with open(AUDIT_LOG, 'a') as f:
                    f.write(f"[{now}] user={user} action=api_run_plugin plugin={plugin} result=fail reason=bad_token\n")
                return
            if os.path.isfile(plugin_path):
                ...
                with open(AUDIT_LOG, 'a') as f:
                    f.write(f"[{now}] user={user} action=api_run_plugin plugin={plugin} result=success\n")
            else:
                self.send_response(404)
                self.end_headers()
                with open(AUDIT_LOG, 'a') as f:
                    f.write(f"[{now}] user={user} action=api_run_plugin plugin={plugin} result=fail reason=not_found\n")
        else:
            super().do_GET()

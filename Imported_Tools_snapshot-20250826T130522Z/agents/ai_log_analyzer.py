        ...
        for line in lines:
            for pattern, label in ANOMALY_PATTERNS:
                if pattern.search(line):
                    recommendations.append(f"[{now}] {log_file}: {label} -> {line.strip()}")
...
with open(AUDIT_LOG, 'a') as f:
    f.write(f"[{now}] user=ai_log_analyzer action=analyze_logs result=success findings={len(recommendations)} suggestions={len(suggested_actions)}\n")

print(f"AI Log Analysis complete. Findings: {len(recommendations)}. Suggestions: {len(suggested_actions)}. See {REPORT_FILE}.")

...
def fix_workflow_completely(file_path):
    ...
    while i < len(lines):
        ...
        # Handle 'on:' block structure
        elif stripped.startswith(('push:', 'pull_request:', 'schedule:', 'workflow_dispatch:')):
            fixed_lines.append('  ' + stripped)
            ...
            fixed_lines.append('    ' + stripped)
        elif stripped.startswith('- ') and any('branches:' in fl or 'paths:' in fl for fl in fixed_lines[-3:]):
            fixed_lines.append('      ' + stripped)
        
        # Handle permissions block
        elif stripped.startswith(('contents:', 'issues:', 'pull-requests:', 'actions:')) and 'permissions:' in ''.join(fixed_lines[-5:]):
            fixed_lines.append('  ' + stripped)
        ...
        # Handle jobs
        elif re.match(r'^[a-zA-Z][a-zA-Z0-9_-]*:', stripped) and 'jobs:' in ''.join(fixed_lines[-10:]):
            # This is a job name
            fixed_lines.append('  ' + stripped)
            
        # Job properties (runs-on, needs, outputs, etc.)
        elif stripped.startswith(('runs-on:', 'needs:', 'outputs:', 'strategy:', 'if:', 'name:')):
            # Check if we're in a job context
            job_context = any(re.match(r'  [a-zA-Z][a-zA-Z0-9_-]*:', fl) for fl in fixed_lines[-10:])
            if job_context:
                fixed_lines.append('    ' + stripped)
            else:
                fixed_lines.append(stripped)
        ...
        # Job outputs and other nested properties
            ...
            if any('outputs:' in fl for fl in fixed_lines[-3:]):
                ...
                fixed_lines.append('      ' + stripped)
            elif 'jobs:' in ''.join(fixed_lines[-10:]) and not any(re.match(r'  [a-zA-Z]', fl) for fl in fixed_lines[-5:]):
                fixed_lines.append('    ' + stripped)
            else:
                fixed_lines.append('  ' + stripped)
        ...
        elif stripped.startswith('- ') and ('steps:' in ''.join(fixed_lines[-10:]) or 
                  any('- name:' in fl or '- uses:' in fl for fl in fixed_lines[-5:])):
            fixed_lines.append('      ' + stripped)
        
        # Step properties (uses:, with:, env:, run:, id:, if:)
        elif stripped.startswith(('uses:', 'with:', 'env:', 'run:', 'id:', 'if:', 'continue-on-error:')):
            # Check if we're in a step context
            in_step = any('- name:' in fl or '- uses:' in fl or '- run:' in fl for fl in fixed_lines[-5:])
            if in_step:
                fixed_lines.append('        ' + stripped)
            else:
                fixed_lines.append('      ' + stripped)
        ...
        # With/env parameters and multiline content
            ...
            if any('with:' in fl or 'env:' in fl for fl in fixed_lines[-3:]):
                if ':' in stripped:
                    ...
                    fixed_lines.append('          ' + stripped)
            elif 'run:' in ''.join(fixed_lines[-3:]) and not stripped.startswith('echo') and not stripped.startswith('if'):
                fixed_lines.append('          ' + stripped)
            elif stripped.startswith(('echo ', 'if ', 'then', 'else', 'fi', 'cat ', 'EOF')):
                fixed_lines.append('          ' + stripped)
            else:

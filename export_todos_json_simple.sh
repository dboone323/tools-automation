#!/bin/bash
# Simple and reliable version of export_todos_json.sh
    ...
    grep -nE --text "TODO|FIXME" "${file}" 2>/dev/null | while IFS=: read -r line_num line; do
      if [[ -n ${line_num} ]]; then
        ...
        jq -c -n --arg file "${file#"${WORKSPACE_DIR}"/}" --arg line "${line_num}" --arg text "${line}" \
          '{file: $file, line: ($line|tonumber), text: $text}' >>"${temp_file}"
      fi
    done || true
  fi
done
...
echo "[" >"${OUTPUT_FILE}"
if [[ -s "${temp_file}" ]]; then
  # Join all JSON objects with commas
  paste -s -d',' "${temp_file}" >>"${OUTPUT_FILE}"
fi
echo "]" >>"${OUTPUT_FILE}"

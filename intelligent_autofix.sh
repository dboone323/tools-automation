...
fix_swiftlint_issues() {
    ...
    if [[ -n ${fixable_rules} ]]; then
        ...
        if swiftlint --fix --format &>/dev/null; then
            swiftlint lint --reporter json 2>/dev/null | jq -r '.[] | select(.severity == "error") | .file' | wc -l >/dev/null 2>&1 || echo "0" >/dev/null
...
run_comprehensive_autofix() {
    ...
    if command -v swiftlint &>/dev/null; then
        ...
        local error_count
        error_count=$(cat "${lint_report}" | grep -c '^File: ')
...
main() {
    ...
    "status")
        ...
        if [[ -d ${BACKUP_DIR} ]]; then
            # shellcheck disable=SC2012  # ls is acceptable for displaying controlled backup directory contents

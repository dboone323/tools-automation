...
def create_pr(repo, branch, pat):
    headers = {'Authorization': 'token ' + pat, 'Accept': 'application/vnd.github+json', 'User-Agent': 'automation-script'}
    payload = {
...
def main():
    ...
    print('Will create PRs for', len(to_process), 'logs (batch=', args.batch, 'sleep=', args.sleep, ')')
    idx = 0
    for path, repo, branch in to_process:
        idx += 1
        print(f'[{idx}/{len(to_process)}] Processing {os.path.basename(path)} -> {repo} [{branch}]')
        try:

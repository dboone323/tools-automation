...
def fix_yaml_workflow(file_path):
    ...
    while i < len(lines):
        ...
        if in_steps and line.strip():
            ...
            if stripped.startswith('- name:') or stripped.startswith('- uses:') or stripped.startswith('- run:') or stripped.startswith('- id:'):
                line = '      ' + stripped
            # Step properties (uses, with, env, etc.) should be indented 8 spaces from job start  
            elif stripped.startswith(('uses:', 'with:', 'env:', 'run:', 'id:', 'if:', 'continue-on-error:', 'timeout-minutes:')):
                line = '        ' + stripped
            # Sub-properties (with parameters, env vars) should be indented 10 spaces from job start
            elif current_indent > 0 and not stripped.startswith('-') and ':' in stripped:
                # Check if previous line was a 'with:' or 'env:' block
                prev_line = fixed_lines[-1].strip() if fixed_lines else ''
                if prev_line.endswith((':')) or any(fixed_lines[-j].strip().startswith(('with:', 'env:')) for j in range(1, min(3, len(fixed_lines)))):
                    line = '          ' + stripped
                else:
                    line = '        ' + stripped
        ...
        # Fix job-level properties
            ...
            stripped = line.strip()
            if stripped.startswith(('runs-on:', 'needs:', 'strategy:', 'outputs:', 'permissions:', 'env:', 'if:')):
                line = '    ' + stripped

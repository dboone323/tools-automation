...
def fix_yaml_file(file_path):
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^  [a-zA-Z-_]+:', line) and 'jobs:' not in line:
            ...
            pass
        elif re.match(r'^    [a-zA-Z-_]+:', line) and not line.strip().startswith('env:') and not line.strip().startswith('with:'):
            # Job properties should be at 4-space indentation, but some need 6
            if any(prop in line for prop in ['runs-on:', 'steps:', 'strategy:', 'env:', 'needs:']):
                if not line.startswith('      '):
                    line = '  ' + line
        
        # Fix step indentation - steps should be 8 spaces, step properties 10 spaces
        if line.strip().startswith('- name:') or line.strip().startswith('- uses:') or line.strip().startswith('- run:'):
            if not line.startswith('        '):

...
while true; do
    ...
    if [[ -n "${TASK_ID}" ]]; then
        ...
        if ! check_resource_limits "search task ${TASK_ID}"; then
        ...
        echo "[$(date)] ${AGENT_NAME}: Creating backup before search operations..." >>"${LOG_FILE}"
        /Users/danielstevens/Desktop/Quantum-workspace/Tools/Automation/agents/backup_manager.sh backup "${PROJECT}" "search_operation_${TASK_ID}" >>"${LOG_FILE}" 2>&1 || true
        ...
        case "${TASK_TYPE}" in
        "search")
            # Run search operations with timeout protection
            echo "[$(date)] ${AGENT_NAME}: Performing code search and indexing..." >>"${LOG_FILE}"
            if ! run_with_timeout 300 "/Users/danielstevens/Desktop/Quantum-workspace/Tools/Automation/master_automation.sh status" "Search operation timed out"; then
                echo "[$(date)] ${AGENT_NAME}: Search operation failed or timed out" >>"${LOG_FILE}"
                TASK_SUCCESS=false
            else
            ...
            ;;
        "index")
            # Run indexing operations with timeout protection
            ...
            " "Indexing operation timed out"; then
                echo "[$(date)] ${AGENT_NAME}: Index update failed or timed out" >>"${LOG_FILE}"
                TASK_SUCCESS=false
            else

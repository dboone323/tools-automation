...
update_task_status() {
    ...
    if command -v jq &>/dev/null; then
        ...
        # append the completed task object to .completed
        | (.completed += (.tasks | map(select(.id==$id))))
      ' "${TASK_QUEUE_FILE}" >"${tq_tmp}" 2>/dev/null && mv "${tq_tmp}" "${TASK_QUEUE_FILE}"
...
release_stale_busy_agents() {
    ...
    # Flip matching agents from busy -> available

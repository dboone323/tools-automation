...
detect_dead_code() {
  ...
  local definitions
  definitions=$(grep -rn "^\s*\(func\|class\|struct\|enum\)" "${project_path}" --include="*.swift" 2>/dev/null | grep -v "^//" || echo "")
  
  local unused_count=0
  
  while IFS=: read -r file line_num declaration; do
    ...
    local ref_count
    ref_count=$(grep -r "\b${name}\b" "${project_path}" --include="*.swift" 2>/dev/null | grep -v "^${file}:${line_num}:" | wc -l | tr -d ' ')
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  case "${1:-daemon}" in
    ...
    cache)
      ...
      ;;
    daemon)
      trap 'update_status "stopped"; log "Optimization Agent stopping..."; exit 0' SIGTERM SIGINT
      main "$@"
      ;;
    *)

...
# Update knowledge base metadata using Python
update_metadata() {
    python3 -c "
import json
import sys
...
try:
    ...
    data['metadata']['total_entries'] = total_entries
    
    # Write to temporary file first, then atomically move
    with tempfile.NamedTemporaryFile(mode='w', dir=os.path.dirname(knowledge_file), delete=False) as temp_file:
        json.dump(data, temp_file, indent=2)
        temp_file.flush()
        os.fsync(temp_file.fileno())
        temp_path = temp_file.name
    
    os.rename(temp_path, knowledge_file)

...
generate_search_report() {
    ...
    if [[ ${effective_scope} == "docs" || ${effective_scope} == "auto" ]]; then
        {
            echo "## Documentation"
            perform_local_search "${query}" "${WORKSPACE}/Documentation" "${max_hits}"
            echo
        } >>"${report_file}"
    fi
    ...
    if [[ ${effective_scope} == "code" || ${effective_scope} == "auto" ]]; then
        {
            echo "## Codebase"
            perform_local_search "${query}" "${WORKSPACE}/Projects" "${max_hits}"
            echo
            echo "## Shared Components"
            perform_local_search "${query}" "${WORKSPACE}/Shared" "${max_hits}"
            echo
        } >>"${report_file}"
    fi
    ...
    if [[ ${effective_scope} == "automation" || ${effective_scope} == "auto" ]]; then
        {
            echo "## Automation"
            perform_local_search "${query}" "${WORKSPACE}/Tools" "${max_hits}"
            echo
        } >>"${report_file}"
    fi

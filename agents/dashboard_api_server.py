...
class DashboardAPIHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        elif self.path == "/" or self.path == "/dashboard" or self.path == "/dashboard.html":
            self.send_response(200)
    ...
    def get_system_metrics(self):
        ...
        try:
            ...
            # Get process count
            process_result = subprocess.run(['ps', 'aux'], capture_output=True, text=True, timeout=5)
            process_count = len(process_result.stdout.strip().split('\n')) - 1 if process_result.returncode == 0 else 500
        ...
        if "agents" in agent_status:
            for agent_name, agent_info in agent_status["agents"].items():
                ...
                if status == "active" and is_running:
                    ...
                    display_status = "failed"  # Restarting but not seen recently = failed
                    ...
                    display_status = "failed"  # Not running and status not recognized = failed
    ...
    def get_agent_description(self, agent_name):
        ...
        return descriptions.get(agent_name, f"{agent_name.replace('_', ' ').replace('.sh', '')} agent")

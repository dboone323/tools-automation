...
let polling = true; let intervalHandle = null;
async function load(){
  try{
...
function scheduleNext(){
  ...
  intervalHandle = setTimeout(()=>{ if(document.getElementById('autorefresh').checked) load(); }, backoff*1000);
}
window.addEventListener('load', ()=>{ load(); scheduleNext(); });
...
async function loadArtifacts(){
  try{
    ...
    let ul = document.getElementById('artlist'); ul.innerHTML='';
    for(let a of arr){

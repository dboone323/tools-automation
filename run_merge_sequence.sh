...
for PR in "${PR_LIST[@]}"; do
  ...
  printf '\n--- Processing PR #%s (log: %s) ---\n' "${PR}" "${LOGFILE}" | tee -a "${LOGFILE}"
  ...
  if [[ -x ./Tools/Automation/master_automation.sh ]]; then
    ...
    if ! ./Tools/Automation/master_automation.sh status >>"${LOGFILE}" 2>&1; then
      echo "automation check returned non-zero" | tee -a "${LOGFILE}"
    fi
  else

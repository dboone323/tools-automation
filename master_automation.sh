...
run_project_automation() {
    ...
    if [[ -f "${project_path}/automation/run_automation.sh" ]]; then
        ...
        (cd "${project_path}" && bash automation/run_automation.sh) || { print_error "${project_name}: automation script failed"; return 1; }
...
init_pods() {
    ...
    (cd "${project_path}" && { if [[ ! -f "Podfile" ]]; then pod init; else pod install; fi }) || { print_error "CocoaPods setup failed for ${project_name}"; return 1; }
...
init_fastlane() {
    ...
    (cd "${project_path}" && { if [[ ! -d "fastlane" ]]; then fastlane init; else print_status "Fastlane already configured"; fi }) || { print_error "Fastlane setup failed for ${project_name}"; return 1; }
...
main() {
    case "${1:-}" in
        list) list_projects ;;
        run) if [[ -n "${2:-}" ]]; then run_project_automation "$2"; else echo "Usage: $0 run <project_name>"; list_projects; exit 1; fi ;;
        all) run_all_automation ;;
        status) show_status ;;
        format) format_code "$2" ;;
        lint) lint_code "$2" ;;
        pods) if [[ -n "${2:-}" ]]; then init_pods "$2"; else echo "Usage: $0 pods <project_name>"; list_projects; exit 1; fi ;;
        fastlane) if [[ -n "${2:-}" ]]; then init_fastlane "$2"; else echo "Usage: $0 fastlane <project_name>"; list_projects; exit 1; fi ;;
        workflow) if [[ -n "${2:-}" ]] && [[ -n "${3:-}" ]]; then "${CODE_DIR}/Tools/Automation/enhanced_workflow.sh" "$2" "$3"; else echo "Usage: $0 workflow <command> <project_name>"; exit 1; fi ;;
        dashboard) "${CODE_DIR}/Tools/Automation/workflow_dashboard.sh" ;;
        unified) "${CODE_DIR}/Tools/Automation/unified_dashboard.sh" ;;
        mcp) if [[ -n "${2:-}" ]]; then "${CODE_DIR}/Tools/Automation/mcp_workflow.sh" "$2" "${3:-}"; else echo "Usage: $0 mcp <command> [project_name]"; exit 1; fi ;;
        ...
        validate) if [[ -n "${2:-}" ]]; then "${CODE_DIR}/Tools/Automation/intelligent_autofix.sh" validate "$2"; else echo "Usage: $0 validate <project_name>"; exit 1; fi ;;
        rollback) if [[ -n "${2:-}" ]]; then "${CODE_DIR}/Tools/Automation/intelligent_autofix.sh" rollback "$2"; else echo "Usage: $0 rollback <project_name>"; exit 1; fi ;;
        enhance) if [[ -n "${2:-}" ]]; then "${CODE_DIR}/Tools/Automation/ai_enhancement_system.sh" "$2" "${3:-}"; else echo "Usage: $0 enhance <command> [project]"; exit 1; fi ;;
        *)

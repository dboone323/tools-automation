...
publish_to_mcp() {
    ...
    critical_count=$(count_issues "$review_text" "Critical")
    local major_count
    major_count=$(count_issues "$review_text" "Major")
    local minor_count
    minor_count=$(count_issues "$review_text" "Minor")
...
review_changes() {
    ...
    # Generate review
    local review_text
    if ! review_text=$(generate_ai_review "$diff_content" "$base_ref" "$head_ref") || [ -z "$review_text" ]; then

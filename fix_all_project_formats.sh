...
PROJECTS_DIR="${PROJECTS_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/Projects}"
...
fix_project() {
  ...
  local xcodeproj_files
  mapfile -t xcodeproj_files < <(find . -maxdepth 2 -name "*.xcodeproj" -type d)

  if [[ ${#xcodeproj_files[@]} -eq 0 ]]; then
    echo -e "${YELLOW}⚠️  No Xcode projects found for ${project_name}${NC}"
    return 0
  fi

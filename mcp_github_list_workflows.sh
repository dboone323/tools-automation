...
# MCP GitHub List Workflows Tool
# Lists GitHub Actions workflows for the repository
...
# List workflows using GitHub CLI
gh workflow list --repo "${OWNER}/${REPO}" --json name,state,path,id | jq -r '.[] | "\(.name) (\(.state)) - \(.path)"'
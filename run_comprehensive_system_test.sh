...
test_load_performance() {
    local test_name="Load Performance Test"

    echo "Running load test (10 seconds at 50 RPS)..."
    if timeout 35 bash -c "source venv/bin/activate && python3 load_test.py --rps 50 --duration 10 --quiet" >load_test.log 2>&1; then
        # Check if load test passed (exit code 0)
        if [ $? -eq 0 ]; then
            log_test "$test_name" "PASS"
            return 0
        else
            log_test "$test_name" "FAIL" "Load test failed - see load_test.log"
            return 1
        fi
    else

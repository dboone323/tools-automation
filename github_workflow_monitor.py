...
def open_issue_for_run(run, title=None, body=None):
    ...
    payload = {'title': title or f"CI failure: {run.get('name')} on {run.get('head_branch')}", 'body': body or f"Workflow failed: {run.get('html_url')}"}
...
def trigger_mcp_debug_run(run):
    ...
    payload = {'workflow': run.get('name'), 'conclusion': run.get('conclusion'), 'url': run.get('html_url'), 'head_branch': run.get('head_branch'), 'run_id': run.get('id'), 'action': 'debug-run'}
    try:
        r = requests.post(f'{MCP_URL}/workflow_alert', json=payload, timeout=10)

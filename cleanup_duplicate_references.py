...
class XcodeprojCleaner:
    ...
    def find_duplicate_file_references(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_children and current_group_uuid:
                ...
                m_child = re.match(r'^\s*([A-F0-9]{24})\s*/\*\s*(.+?)\s*\*/\s*,\s*$', line)
                if m_child:
                    child_uuid = m_child.group(1)
                    filename = m_child.group(2)
                    occurrences[child_uuid].append({'line': i, 'group': current_group_uuid, 'filename': filename})
    ...
    def remove_duplicate_build_files(self, duplicates):
        ...
        for file_ref_uuid, build_files in duplicates.items():
            ...
            for build_file in build_files[1:]:
                ...
                print(f"  ❌ Removing duplicate build file: {filename} (line {line_num + 1})")
                removed_count += 1
    ...
    def remove_duplicate_file_references_from_groups(self, duplicates):
        ...
        for file_uuid, occurrences in duplicates.items():
            ...
            for occurrence in occurrences[1:]:
                ...
                lines_to_remove.add(line_num)
                print(f"  ❌ Removing duplicate group reference: {filename} (line {line_num + 1})")
                removed_count += 1
    ...
    def clean(self):
        ...
        print(f"   If there are issues, restore from: {self.pbxproj_path.with_suffix('.pbxproj.backup')}")


def verify_project(xcodeproj_path):
    """Verify the project by running xcodebuild -list"""
    print(f"\n🔍 Verifying project: {xcodeproj_path}")
    result = os.system(f'xcodebuild -project "{xcodeproj_path}" -list 2>&1 | grep -c "member of multiple groups"')
    return result == 0
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print("  python3 cleanup_duplicate_references.py ../Projects/HabitQuest/HabitQuest.xcodeproj")
        sys.exit(1)

...
def register():
    ...
    cid = f'local-controller-{PROJECT_NAME}' if PROJECT_NAME else 'local-controller-unknown'
    session.headers.update({'X-Client-Id': cid})
    backoff = 1.0
    for attempt in range(6):
        try:
            session.post(f'{MCP_URL}/register', json={'agent': AGENT_NAME, 'capabilities': ['controller']}, timeout=5)
...
_session.headers.update({'X-Client-Id': f'local-controller-{PROJECT_NAME}' if PROJECT_NAME else 'local-controller-unknown'})
...
def send_heartbeat(project=None):
    try:
        _session.post(f'{MCP_URL}/heartbeat', json={'agent': AGENT_NAME, 'project': project}, timeout=4)
...
def execute_task(task):
    ...
    try:
        ...
        r = _safe_request(requests.post, f'{MCP_URL}/execute_task', json={'task_id': task_id}, timeout=5, headers=_session.headers)
        ...
        while True:
            try:
                s = _safe_request(requests.get, f'{MCP_URL}/status', timeout=6, headers=_session.headers).json()
            except Exception:
...
def upload_artifacts(task):
    ...
    if uploader.exists() and os.access(uploader, os.X_OK):
        ...
        base = Path(ARTIFACT_DIR) / f"{proj}_{tid}_{ts}"
        try:
            requests.get(f'{MCP_URL}/status', timeout=3)  # quick check to ensure MCP reachable
        except Exception:
            pass
        ...
        threading.Thread(target=run_uploader, args=(uploader, base), daemon=True).start()

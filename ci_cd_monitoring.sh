...
# Get workflow status
get_workflow_status() {
    local report_file="${REPORTS_DIR}/workflow_status_$(date +%Y%m%d_%H%M%S).json"
...
get_recent_runs() {
    local limit="${1:-20}"
    local report_file="${REPORTS_DIR}/recent_runs_$(date +%Y%m%d_%H%M%S).json"
...
analyze_workflow_health() {
    local status_file="$1"
    local runs_file="$2"
    local analysis_file="${REPORTS_DIR}/health_analysis_$(date +%Y%m%d_%H%M%S).md"
...
main "$@"</content>
<parameter name="filePath">/Users/danielstevens/Desktop/Quantum-workspace/Tools/Automation/ci_cd_monitoring.sh

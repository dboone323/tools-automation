...
elif [ "$TOTAL_SCRIPTS" -eq 0 ]; then
    log_security_pass "No shell scripts found"
else
    MISSING_EXEC=$((TOTAL_SCRIPTS - EXECUTABLE_SCRIPTS))
    if [ "$MISSING_EXEC" -gt 10 ]; then # Only warn if many scripts are missing permissions
        log_security_issue "LOW" "Some scripts without execute permissions" "$MISSING_EXEC scripts lack execute permission (may be intentional)"
    else
        log_security_pass "Script permissions appear appropriate"
    fi
fi
...
if grep -r "SELECT.*+.*WHERE\|INSERT.*+.*VALUES\|UPDATE.*+.*SET" --include="*.py" --include="*.js" . >/dev/null 2>&1; then
    SQL_INJECTION_FILES=$(grep -r -l "SELECT.*+.*WHERE\|INSERT.*+.*VALUES\|UPDATE.*+.*SET" --include="*.py" --include="*.js" . 2>/dev/null | wc -l)
    log_security_issue "MEDIUM" "Potential SQL injection vulnerability" "$SQL_INJECTION_FILES files use string concatenation in SQL queries"
else
    log_security_pass "No obvious SQL injection vulnerabilities found"
fi

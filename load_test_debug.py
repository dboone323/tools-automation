...
async def main():
    ...
    try:
        ...
        # Exit with appropriate code based on performance (DEBUG MODE - more lenient)
        success_rate = stats["success_rate"]
        rps_achievement = stats["rps_achievement"]

        if success_rate >= 0.90 and rps_achievement >= 0.70:
            print("✅ Load test PASSED (DEBUG MODE)")
            sys.exit(0)
        else:
            print("❌ Load test FAILED (DEBUG MODE)")
            print(

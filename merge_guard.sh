...
check_validation_reports() {
    ...
    for report in "${VALIDATION_REPORTS_DIR}"/*.json; do
        ...
        # Check report age
        local report_time
        report_time=$(stat -f %m "$report" 2>/dev/null || stat -c %Y "$report" 2>/dev/null)
        ...
        overall_status=$(jq -r '.overall_status // "unknown"' "$report")
        ...
        if [ "$overall_status" = "passed" ]; then
            ...
            local lint_status
            lint_status=$(jq -r '.lint.status // "unknown"' "$report")
            local format_status
            format_status=$(jq -r '.format.status // "unknown"' "$report")
            local build_status
            build_status=$(jq -r '.build.status // "unknown"' "$report")
            ...
            if [ "$lint_status" = "failed" ]; then
                local lint_errors
                lint_errors=$(jq -r '.lint.errors // 0' "$report")
                local lint_warnings
                lint_warnings=$(jq -r '.lint.warnings // 0' "$report")
...
check_mcp_alerts() {
    ...
    # Get alerts from MCP
    local alerts_json
    if ! alerts_json=$(curl -sf "${MCP_SERVER}/status" 2>/dev/null); then
    ...
    local alert_count
    alert_count=$(echo "$alerts_json" | jq '.alerts | length')
    ...
    while IFS= read -r alert; do
        ...
        local alert_time
        alert_time=$(date -j -f "%Y-%m-%dT%H:%M:%S" "${timestamp%.*}" +%s 2>/dev/null || echo "$current_time")
...
check_ai_review() {
    ...
    local latest_review
    latest_review=$(ls -t "${AI_REVIEWS_DIR}"/review_*.md 2>/dev/null | head -1)
    ...
    current_time=$(date +%s)
    local review_time
    review_time=$(stat -f %m "$latest_review" 2>/dev/null || stat -c %Y "$latest_review" 2>/dev/null)
    ...
    local critical
    critical=$(grep -i "Critical Issues:" "$latest_review" | grep -oE '[0-9]+' | head -1 || echo "0")
    local major
    major=$(grep -i "Major Issues:" "$latest_review" | grep -oE '[0-9]+' | head -1 || echo "0")
    local minor
    minor=$(grep -i "Minor Issues:" "$latest_review" | grep -oE '[0-9]+' | head -1 || echo "0")

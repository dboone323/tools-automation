#!/bin/bash
# Fixed version of export_todos_json.sh
    ...
    grep -nE --text "TODO|FIXME" "${file}" 2>/dev/null | while IFS=: read -r line_num line; do
      if [[ -n ${line_num} ]]; then
        jq -n --arg file "${file#"${WORKSPACE_DIR}"/}" --arg line "${line_num}" --arg text "${line}" \
          '{file: $file, line: ($line|tonumber), text: $text}' >>"${temp_file}.items"
      fi
    done || true
  fi
done

# Combine all items into proper JSON array
if [[ -f "${temp_file}.items" ]]; then
  # Add commas between items
  sed '$!s/$/,/' "${temp_file}.items" >>"${temp_file}"
fi

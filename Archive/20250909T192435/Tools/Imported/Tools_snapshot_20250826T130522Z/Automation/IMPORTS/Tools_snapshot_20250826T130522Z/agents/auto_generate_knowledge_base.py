...
for fname in sorted(os.listdir(AGENTS_DIR)):
    if fname.endswith(('.sh', '.py')) and not fname.startswith(('onboard', 'distributed_launcher', 'distributed_health_check', 'auto_generate_knowledge_base')):
        path = os.path.join(AGENTS_DIR, fname)
        with open(path) as f:
            first_lines = [next(f) for _ in range(5)]
        desc = next((re.sub(r'^# ?','',l).strip() for l in first_lines if l.strip().startswith('#')), fname)

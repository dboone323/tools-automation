...
missing_files = [
    ("Accessibility/AccessibilityEnhancements.swift", "AccessibilityEnhancements.swift"),
...
def add_files_to_project():
    ...
    sources_section = re.search(r'(68F097112DC01EFA00092697 /\* Sources \*/ = \{.*?files = \(.*?\);)', content, re.DOTALL)
    ...
    for entry in new_build_files:
        modified_content = modified_content[:build_file_end] + entry + "\n" + modified_content[build_file_end:]
        build_file_end += len(entry) + 1
    
    # Add file references
    file_ref_end = re.search(r'/\* End PBXFileReference section \*/', modified_content).start()
    for entry in new_file_refs:
        modified_content = modified_content[:file_ref_end] + entry + "\n\t\t" + modified_content[file_ref_end:]
        file_ref_end += len(entry) + 3
    
    # Add to sources
    sources_files_end = re.search(r'68F097592DC0232400092697 /\* MainTabView\.swift in Sources \*/,', modified_content).end()
    for entry in new_source_entries:
        modified_content = modified_content[:sources_files_end] + "\n" + entry + modified_content[sources_files_end:]

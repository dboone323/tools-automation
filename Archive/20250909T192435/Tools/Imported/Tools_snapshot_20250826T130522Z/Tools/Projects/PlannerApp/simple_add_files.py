...
result = subprocess.run(["find", ".", "-name", "*.swift", "-not", "-path", "./DerivedData/*"], 
                       capture_output=True, text=True)
files_on_disk = [f.replace("./", "") for f in result.stdout.strip().split("\n") if f.strip()]
...
for file_path in files_on_disk:
    ...
    if file_path in files_in_project or filename in [os.path.basename(f) for f in files_in_project]:
        continue
    
    # Skip backup versions and test files for now
    if any(suffix in filename for suffix in ['_Working', '_Final', '_Minimal', '_Simple', '_New', 'Tests']):
        continue
...
if build_section:
    ...
    new_build_section = (build_section_text[:insertion_point] + 
...
if file_ref_section:
    ...
    new_file_section = (file_section_text[:insertion_point] + 
                       "\n".join(new_file_refs) + "\n\t\t" +
...
if sources_section:
    ...
    new_sources_section = (sources_text[:insertion_point] + 
